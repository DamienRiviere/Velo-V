...
const createCanvas = {
  ...
  mouseY: 0,
  lastX: -1,
  // Variables pour la précédente position de la souris
  lastY: -1,
  mouseDown: false,
  // Variable pour vérifer que le boutton de la souris soit appuyée pour commencer à dessiner
  context: null,
  ...
  drawLine(x, y) {
    // Pour dessiner des traits
    if (this.lastX == -1) {
      // Si c'est un nouveau chemin
  ...
  draw() {
    ...
    canvas.addEventListener('mousemove', function (e) {
      ...
      if (createCanvas.mouseDown === true) {
        // On regarde si le bouton est down
        createCanvas.drawLine(createCanvas.mouseX, createCanvas.mouseY); // Et s'il l'est on dessine
      }
    }, false);
    window.addEventListener('mouseup', function () {
      // Quand le bouton de la souris n'est plus down
    ...
    }, false); // Même chose mais pour les tablettes et smartphones
