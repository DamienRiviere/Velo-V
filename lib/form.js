...
const form = {
  ...
  // Bouton pour faire apparaître le formulaire de réservation
  buttonReservationSuccess: document.getElementById("reservation-success"),
  // Bouton pour valider la réservation
  buttonReservationDelete: document.getElementById("delete-reservation"),
  // Bouton pour annuler la réservation en cours
  mapLegend: document.getElementById("map-legend"),
  // Légende de la carte
  sectionReservation: document.getElementById("section-reservation"),
  // Section de la réservation
  canvasClean: document.getElementById("canvas-delete"),
  // Bouton pour nettoyer le canvas
  alertForm: document.getElementById("alert-form"),
  // Message d'alerte suivant la situation
  sectionForm: document.getElementById("section-form"),
  // La section entière d'une réservation
  reservationTitle: document.getElementById("reservation-title"),
  // Titre de la réservation en cours
  reservationTime: document.getElementById("reservation-timer"),
  // Temps de la réservation en cours
  reservationContainer: document.getElementById("reservation-container"),
  // Container de la réservation en cours
  x: null,
  ...
  reservationSuccess(station) {
    this.buttonReservationSuccess.addEventListener("click", function () {
      ...
      _canvas.createCanvas.context.clearRect(0, 0, canvas.width, canvas.height);

      sessionStorage.setItem("name", station); // Gestion des différents éléments
      ...
      form.mapLegend.style.display = "block"; // On déclenche le décompte de la réservation
  ...
  reservationTimer(dateEnCours) {
    ...
    let dateFin; // On utilise la valeur "dateEnCours" dans la variable "dateFin"

    if (dateEnCours) {
      dateFin = new Date().getTime() + Number(dateEnCours); // Si dateEnCours est vide on stocke la date actuel + 20 minutes
    } else {
    ...
    this.x = setInterval(function () {
      ...
      const distance = dateFin - dateActuel; // On transforme les millisecondes en minutes et secondes

      let minutes = Math.floor(distance % (1000 * 60 * 60) / (1000 * 60));
      let secondes = Math.floor(distance % (1000 * 60) / 1000); // On stock la date actuel en milliseconde

      sessionStorage.setItem("temps", distance); // Permet d'afficher la réservation dans le container de la réservation en cours

      form.reservationSuccessDisplay(minutes, secondes); // Si le décompte arrive à 0, la réservation expire
  ...
  reservationSuccessDisplay(minutes, secondes) {
    ...
    form.reservationTime.innerHTML = "Vous avez r\xE9serv\xE9 1 v\xE9lib \xE0 la station <span class=\"font-weight-bold\">".concat(sessionStorage.name, "</span>. <br> Expire dans : <span class=\"font-weight-bold\">").concat(minutes, " minute(s) et ").concat(secondes, " seconde(s)</span>.");
  },
  ...
  deleteReservation() {
    this.buttonReservationDelete.addEventListener("click", function () {
      ...
      form.alertForm.innerHTML = "\n                <i class=\"fas fa-exclamation-triangle\"></i>\n                Veuillez s\xE9lectionner une station !\n            ";
  ...
  webStorage() {
    ...
    let dateEnCours = sessionStorage.getItem("temps"); // Si la valeur de "temps" est supérieur à 0, on reprends la réservation en cours
