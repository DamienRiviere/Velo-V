...
const markerManager = {
  ...
  createMarkerClusterer() {
    const markerCluster = new MarkerClusterer(_map.googleMaps.map, markerManager.markers, {
  ...
  showStation(station) {
    markerManager.marker.addListener("click", function () {
      _map.googleMaps.map.panTo(station.position);

      _map.googleMaps.map.setZoom(17); // On affiche la section de réservation uniquement s'il n'y a pas de réservation en cours


      if (!sessionStorage.getItem("temps") || sessionStorage.getItem("temps") < 0) {
        _map.googleMaps.form.style.display = "block";
        ...
        document.getElementById("station-street").textContent = station.address; // Gestion de l'affichage du statut de la station (OUVERT/FERMER)

        markerManager.showStatut(station); // Gestion de l'affichage des vélibs en fonction de leur nombre

        markerManager.showAvailableBike(station);

        _form.form.reservationSuccess(station.name);
      }
    });
  },
  ...
  showAvailableBike(station) {
    if (station.available_bikes > 0) {
      this.stationBike.textContent = "".concat(station.available_bikes, " v\xE9lib(s) disponibles.");
      ...
      this.stationBike.textContent = "".concat(station.available_bikes, " v\xE9lib disponible.");
